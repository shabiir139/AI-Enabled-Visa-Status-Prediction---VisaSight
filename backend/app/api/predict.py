...
def predict_status(request: PredictRequest):
    ...
    """
    # Optimization: Using standard `def` (not `async def`) to run CPU-bound ML inference
    # in a thread pool, preventing blocking of the main event loop.
...
def get_prediction_explanation(case_id: str):
    ...
    """
    # Optimization: Using standard `def` (not `async def`) to run CPU-bound SHAP explanation
    # in a thread pool, preventing blocking of the main event loop.
