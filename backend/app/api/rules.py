...
async def list_visa_rules(
    ...
    try:
        query = supabase.table("visa_rules").select("*", count="exact").eq("is_active", True)  # ⚡ Bolt: Combine Supabase pagination queries into a single round-trip using count='exact'
        ...
        # Get total count directly from the primary query result
