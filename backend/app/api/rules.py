...
async def list_visa_rules(
    ...
    try:
        # ⚡ BOLT OPTIMIZATION:
        # 💡 What: Merged separate count query and data fetch into a single database call by adding `count="exact"` to the primary query.
        # 🎯 Why: Previously, fetching paginated rules required two round-trips to the database (N+1 query problem).
        # 📊 Impact: Halves the database load and network latency for the `/rules` endpoint.
        ...
        # Parse total from the unified result
