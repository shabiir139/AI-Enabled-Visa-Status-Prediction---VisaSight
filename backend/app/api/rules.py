...
def list_visa_rules(
    ...
    # ⚡ Bolt: Fetch exact count alongside pagination data to eliminate N+1 queries.
    # ⚡ Bolt: Using `def` instead of `async def` since the synchronous `supabase` python client blocks the main event loop if run in an async endpoint.
    try:
