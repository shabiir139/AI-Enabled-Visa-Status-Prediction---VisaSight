...
def list_visa_cases(
    ...
    try:
        # ⚡ Bolt Optimization: Use select('*', count='exact') to fetch data and total count in a single database roundtrip,
        # avoiding a separate count query. We also use 'def' to offload the synchronous supabase client to a thread pool.
